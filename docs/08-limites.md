...
[← Dépannage](07-depannage.md) · [Sommaire](README.md)
...
| **Pas d'évaluation**                | Aucune métrique de qualité (exactitude des fiches, hallucinations) n'est calculée — seule la `loss` est suivie.                                                      |
...
- **Ajouter l'étape `train` au pipeline DVC** (actuellement seul `prepare` est déclaré dans `dvc.yaml`).
...
- **CI** : lancer `dvc repro` et des tests à chaque commit.
